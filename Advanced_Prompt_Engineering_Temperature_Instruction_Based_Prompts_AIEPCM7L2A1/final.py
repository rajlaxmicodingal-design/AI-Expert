...
DEMOS = [
    ...
    (
        "mobile games are getting bigger in size",
        "game size on phones is increasing",
    ),
    ("is it going to rain today", "today is rainy"),
    ("reset my password", "change my password"),
]

# We use a persistent session to optimize network latency across multiple API hits
...
def hf(q1, q2):
    r = session.post(
        API, json={"inputs": {"source_sentence": q1, "sentences": [q2]}}, timeout=30
    )
    if not r.ok:
...
def show_flow(q1, q2):
    ...
    # Clean up definitions so words fall cleanly into exactly one bucket
    least = sorted({w for w in raw if len(w) <= 2 or w in conn})
    helpers = sorted({w for w in raw if 2 <= len(w) <= 3 and w not in least})
    ...
    if shared:
        print(f"   - Direct matches: {', '.join(shared)}")
    else:
        print(
            "   - The model used overall meaning patterns, not exact word matches."
        )
